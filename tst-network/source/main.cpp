...
task<Report> TestOrchid(const S<Origin> &origin, std::string name, const S<Network> &network, const char *address, const Chain &chain, const Secret &secret, const Address &funder) {
    ...
    co_return co_await Using<BufferSink<Remote>>([&](BufferSink<Remote> &remote) -> task<Report> {
        ...
        auto &client(*co_await network->Connect(remote, origin, provider, chain, "0xb02396f06CC894834b7934ecF8c8E5Ab5C1d12F1", secret, funder, nullptr));
        remote.Open();
...
int Main(int argc, const char *const argv[]) {
    ...
    const auto network(Wait(Network::Create(milliseconds, chain, directory, location)));
