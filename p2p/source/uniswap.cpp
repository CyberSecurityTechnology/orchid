...
task<Float> Uniswap(const Chain &chain, const Block &block, const Address &pair) {
    ...
    const auto [reserve0after, reserve1after, after] = co_await getReserves_.Call(chain, block.height_, pair, 90000);
#if 0
...
task<S<Updated<Fiat>>> UniswapFiat(unsigned milliseconds, S<Chain> chain) {
    co_return co_await Opened(Updating(60*1000, [chain = std::move(chain)]() -> task<Fiat> {
        const auto block(co_await chain->Header("latest"));
        const auto [usdc_weth, oxt_weth] = *co_await Parallel(
            Uniswap(*chain, block, "0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc"),
            Uniswap(*chain, block, "0x9b533f1ceaa5ceb7e5b8994ef16499e47a66312d"));
