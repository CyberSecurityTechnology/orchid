    ...
    public Endpoint
{
  ...
  public:
    Chain(Endpoint endpoint, const Flags &flags, const uint256_t &chain) :
    ...
    static task<S<Chain>> Create(Endpoint endpoint, const Flags &flags, const uint256_t &chain) {
        co_return Break<Chain>(std::move(endpoint), flags, chain);
    }

    static task<S<Chain>> Create(Endpoint endpoint, const Flags &flags) {
        const auto chain(uint256_t((co_await endpoint("eth_chainId", {})).asString()));
        co_return co_await Create(std::move(endpoint), flags, chain);
    }
