    ...
    public Endpoint
{
  ...
  public:
    Chain(Endpoint endpoint, const uint256_t &chain, Flags flags = Flags()) :
    ...
    static task<S<Chain>> Create(Endpoint endpoint, Flags flags = Flags()) {
        const auto chain(uint256_t((co_await endpoint("eth_chainId", {})).asString()));
        co_return Make<Chain>(std::move(endpoint), chain, std::move(flags));
    }
